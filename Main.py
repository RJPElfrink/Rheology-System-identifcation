...
t_steady = np.linspace(0,T,N,endpoint=False)
A_strain = np.ones_like(f)                           # Strain rate per frequency
...
u_transient_1, phase_u=rhs.multisine(N,f_s, f_0, [J1,J2],P=P,phase_response='Schroeder',normalize='None',time_domain=True)
...
u_transient_t, phase_u=rhs.multisine(N,f_s, f_0, [J1,J2],P=P,phase_response='Schroeder',normalize='None',time_domain=False)
...
U_steady, Y_steady,G_steady = rhs.fft_normalization(u_steady,y_steady,normalize="STDev")
...
transient = np.exp(-t_transient/Lambda_constant) # set to zero to exclude transient

# Figure of the created multisine signals frequency domain in Decibel
plt.plot(t_transient,np.tile(y_steady,P)-y_transient,label='y-y$_p$')
plt.plot(t_transient,transient,label='analytical transient')
plt.ylabel('y-y$_p$')
plt.legend()
plt.xlabel('Time[s]')
...
u_transient=rhs.chirp_exponential(t_steady,[k_1,k_2],f_s,N,P)

# Compute the response
t_out,y_out, _ = signal.lsim(s1, U=u_transient, T=t_transient)

#U, Y, G = rhs.fft_normalization(u_steady,y_steady,normalize='Amplitude')
