...
freq = double(ExcitedHarm) * double(fs) / double(N);
% frequency excited harmonics
...
SelectExcited = double(P)*double(ExcitedHarm) + 1;                              % selection of the excited harmonics
Zfft = fft([data.y; data.u], [], 2)/double(N*P);                      % scaling DTF s.t. the correct Fourier coefficients are recovered
