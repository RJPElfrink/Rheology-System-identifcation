...
fact_crest = rhs.crest_fac(u_0_transient)
fact_effic = rhs.power_efficiency(fft(u_0_transient),band_range)
fact_loss  = rhs.power_loss(fft(u_0_transient),band_range)
...
G_0=G_0[f_log]
G_export=G_ML[:int(band_range[1])]
bias_export=G_0-G_export
var_export=np.var(G_etfe_m,axis=0)
var_export=var_export[:int(band_range[1])]
